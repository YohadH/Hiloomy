...
const OPS = new Set(["move", "split", "merge", "exclude", "include", "link_event", "unlink_event", "link_entity", "unlink_entity", "unlink_kind", "set_fact", "reset"]);
...
export async function POST(request: Request, context: { params: Promise<{ sheetId: string }> }) {
  try {
    ...
    if ((body.op === "link_entity" || body.op === "unlink_entity" || body.op === "unlink_kind" || body.op === "set_fact") && typeof (body as { initiativeId?: string }).initiativeId === "string") {
