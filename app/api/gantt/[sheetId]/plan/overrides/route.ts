...
const OPS = new Set(["move", "split", "merge", "exclude", "include", "link_event", "unlink_event", "link_entity", "unlink_entity", "unlink_kind", "reset"]);
...
export async function POST(request: Request, context: { params: Promise<{ sheetId: string }> }) {
  try {
    ...
    if ((body.op === "link_entity" || body.op === "unlink_entity" || body.op === "unlink_kind") && typeof (body as { initiativeId?: string }).initiativeId === "string") {
