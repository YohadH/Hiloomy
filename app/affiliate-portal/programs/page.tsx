      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
        <Card>
          <CardHeader className="flex flex-row items-start justify-between gap-4">
            <div>
              <CardTitle>{repairMojibake(program.name)}</CardTitle>
          ...
          <CardContent className="space-y-4">
            {/* Orders/sales follow the SELECTED window (F-082 — the old
                lifetime sums showed 45× the store's period revenue under a
                date picker); the affiliate count is all-time and says so. */}
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
                <p className="text-sm text-muted-foreground">{lang("שותפות (סה״כ מאז ומעולם)", "Affiliates (all-time)")}</p>
                <p className="mt-2 text-2xl font-semibold">{formatNumber(program.affiliates)}</p>
              </div>
              <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
                <p className="text-sm text-muted-foreground">{lang("הזמנות בטווח שנבחר", "Orders (selected window)")}</p>
                <p className="mt-2 text-2xl font-semibold">{formatNumber(program.orders)}</p>
              </div>
              <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
                <p className="text-sm text-muted-foreground">{lang("מכירות בטווח שנבחר", "Sales (selected window)")}</p>
        ...
        {/* Returning-commission policy card HIDDEN per the owner (F-085).
            The feature and its stored policy value stay intact — only the
            UI is hidden; the commission-leakage leak's recommended action
            still routes to a working API. Un-hide by restoring
            <ReturningPolicyCard .../> here. */}
      </section>
