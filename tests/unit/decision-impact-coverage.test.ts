...
test("business context: windows come only from the plan's named events; the most urgent decision is the one inside the soonest window", () => {
  ...
  // "ראש השנה" and "סוכות" initiatives are SUGGESTED (unconfirmed) links under their calendar events,
  // so they are not bare campaign windows; the text anchor and the unattached row are neither.
  assert.deepEqual(ctx.windows, []);
  const rosh = ctx.events.find((e) => e.calendarEvent.id === "rosh_hashanah_2026")!;
  assert.equal(rosh.calendarEvent.startDate, "2026-09-11"); // calendar truth, not the campaign's Sep 2–13
  assert.deepEqual(rosh.linkedInitiatives.map((l) => [l.id, l.linkState]), [["i2", "suggested"], ["i1", "suggested"]]);
  assert.equal(rosh.linkedInitiatives.find((l) => l.id === "i1")!.openDecisionCount, 1);
  assert.equal(rosh.decisionUrgency, null); // suggested links never drive urgency
  const sukkot = ctx.events.find((e) => e.calendarEvent.id === "sukkot_2026")!;
  assert.deepEqual(sukkot.linkedInitiatives.map((l) => l.id), ["i3"]);
...
test("business context windows carry starts/ends, related decisions, thin-stock products and the open question; the summary line is deterministic", () => {
    ...
    initiatives: [
      {
        id: "i1",
        kind: "move",
        anchor: { kind: "event", label: "מבצע סתיו" },
        start: "2026-09-10",
        end: "2026-09-13",
      ...
      { id: "i2", kind: "move", anchor: { kind: "event", label: "מבצע סתיו" }, start: "2026-09-02", end: "2026-09-13", products: [product("p1", 6), product("p4", 2)], relatedDecisions: [] },
      { id: "i3", kind: "move", anchor: { kind: "event", label: "יריד אוקטובר" }, start: "2026-09-20", end: "2026-09-30", products: [], relatedDecisions: [] }
    ]
  } as unknown as PlanView;
  ...
  // The calendar event (system fact, Sep 10 → "starts tomorrow") leads; the unlinked campaign follows in campaign language.
  assert.equal(ctx.summaryLine.en, "Rosh Hashanah starts tomorrow, מבצע סתיו campaign is under way, 2 initiatives are active, Meta is live (18 campaigns), 0 inventory risks are open, 1 plan decision requires attention.");
  // The three dates stay apart: the card is the CAMPAIGN, the holiday has no source, the decision window is the hook's.
  assert.equal(rosh.campaignTitle.en, "מבצע סתיו campaign");
  assert.equal(rosh.title, "מבצע סתיו");
  ...
  assert.equal(bare.summaryLine.en, "Rosh Hashanah starts tomorrow, 0 inventory risks are open.");
});
...
test("decision window comes from the soonest hook still open, never from the campaign dates; a launch is titled as a launch", () => {
  ...
  const plan = {
    ...
    today: "2026-09-10",
    initiatives: [
      { id: "i1", kind: "move", anchor: { kind: "event", label: "מבצע סתיו" }, start: "2026-09-01", end: "2026-09-30", products: [], relatedDecisions: [], decisionHooks: [hook("past", "2026-09-01", "2026-09-05"), hook("late", "2026-09-20", "2026-09-22"), hook("next", "2026-09-12", "2026-09-14")] },
  ...
  const rosh = ctx.windows[0];
  assert.equal(rosh.campaignTitle.he, "קמפיין מבצע סתיו");
